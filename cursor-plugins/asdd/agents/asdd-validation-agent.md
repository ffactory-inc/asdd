...
| Input                      | Path | Required |
|----------------------------|---|---|
| Requirements specification | `.cursor/specs/[spec-name]/requirements.md` | Mandatory |
...
| Prior validation report    | `.cursor/specs/[spec-name]/spec-validation-report.md` | Optional |
...
Generate or update:
- `.cursor/specs/[spec-name]/spec-validation-report.md`
- `.cursor/state/manifest.json` (Update proposal)

Do not modify `requirements.md`, `capability.md`, or `domain-model.md`.

## State Transition

At the end of your execution, propose an update to `.cursor/state/manifest.json`:
