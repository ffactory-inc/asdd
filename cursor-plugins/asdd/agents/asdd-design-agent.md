...
| Input                  | Path | Required |
|------------------------|---|---|
| Validated requirements | `.cursor/specs/[spec-name]/requirements.md` | Mandatory — must be status READY |
| State Manifest         | `.cursor/state/manifest.json` | Mandatory |
| Validation report      | `.cursor/specs/[spec-name]/spec-validation-report.md` | Mandatory — must be PASSED or PASSED_WITH_WARNINGS |
...
- **Do not make** security design decisions that contradict `.cursor/rules/security-rules.md`.
...
At the end of your execution, you must propose an update to `.cursor/state/manifest.json`:
...
Generate `.cursor/specs/[spec-name]/design.md` following this exact structure:
...
- Compliance with `.cursor/rules/security-rules.md`.
