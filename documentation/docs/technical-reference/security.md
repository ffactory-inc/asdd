...
Security in ASDD is **not based on markdown rules alone**. Steering rules stored in `.asdd/steering/` are the *specification* — enforcement happens at multiple automated layers, validated by the Security Agent before every deployment.
...
```mermaid
flowchart TD
    Rules[".asdd/steering/security-rules.md\nHuman-readable source of truth"]
...
Every rule defined in `.asdd/steering/security-rules.md` is enforced at three stages:
...
Steering rules in `.asdd/steering/security-rules.md` have a strict modification protocol:
