...
> Contributors: AI peer review (Claude, Anthropic)
...
### Value 1 — Validated intent over generated code `[shared]`
...
### Value 2 — Human authority over agent autonomy `[human-owned]`
...
### Value 3 — Continuous trust loops over periodic reviews `[shared]`
...
### Value 4 — Adaptive execution over rigid planning `[agent-driven]`
...
### 05 — The Hallucination Budget ✦ *revised*

Agent hallucination is not a random defect — it is a measurable signal. Every agent pipeline defines a **Hallucination Budget**: a maximum tolerable rate of low-confidence outputs per execution wave. When an agent's Confidence Score drops below threshold consistently, the framework triggers an automatic **Discovery Re-entry** — returning the slice to Phase −1 for requirement re-validation.

This prevents the system from wasting implementation resources on flawed execution when the root cause is ambiguous requirements upstream. Hallucination is an input problem, not an output problem.

> **Mechanism:** `confidence_score < threshold` on N consecutive artifacts → pipeline halts → Knowledge Agent flags `DISCOVERY_REENTRY` → slice status reset to `SPEC` → PO + TL notified.
...
Small squads of humans plus agents outperform large human-only teams. Optimise for squad autonomy, domain ownership, and agent augmentation — not headcount. A typical squad: 3 humans + 6–10 AI agents. Scale squads, not people.
...
### 09 — Agents receive only what they need to know ✦ *revised*

Agents degrade in long, accumulating context windows. The principle is minimal, purposeful context as a quality value — not a performance trick. The **Knowledge Agent** enforces this through **Dynamic Context Injection**: at each phase gate, it injects only the Ubiquitous Language terms, Steering Rules, and domain constraints directly relevant to the current task. Everything else is withheld.

A clean context window is as important as clean code. Design pipelines that work with how agents actually function, not against it.

> **Mechanism:** Knowledge Agent reads the active slice's `phase_data`, identifies relevant steering files and domain terms, constructs a minimal context payload, and injects it into the spawned sub-agent's prompt — never the full manifest.

---

### 10 — Every agent action leaves an audit trail

Confidence scores, state transitions, and governance decisions are logged and human-readable. You cannot govern what you cannot observe. An audit trail of outputs is necessary but not sufficient — transparency of *reasoning* is required for meaningful human oversight. See Principle 13.
...
### 12 — Agent-Agent conflict resolution escalates to humans ✦ *revised*

Parallel execution waves will produce agent-agent conflicts: overlapping domain model modifications, contradictory architecture proposals, race conditions on the State Manifest. The **Knowledge Agent** is the first arbitration layer — it detects conflicts via the Manifest and attempts resolution using Steering Rules.
...
> **Mechanism:** Conflict detected → Knowledge Agent locks affected slice → applies Steering Rules → if unresolved within threshold, sets slice status to `BLOCKED` → Tech Lead notified with conflict diff for human arbitration.

---

### 13 — Agents must explain their reasoning, not just their output ✦ *new*
...
Every agent must emit a **Reasoning Trace** alongside its artifact: the key assumptions it made, the requirements it prioritised, the alternatives it considered and rejected, and the uncertainty factors that reduced its confidence score. Phase gate sign-off by the Tech Lead requires the Reasoning Trace, not just the artifact.

> **Mechanism:** Each agent appends a `reasoning_trace.md` to its output. The phase gate checklist requires TL acknowledgment of the trace before the gate passes. Traces are stored in `.kiro/traces/` and linked in the State Manifest.

---

### 14 — Governance is as important as capability ✦ *new*
...
| DevOps Agent | CI/CD gates, infrastructure automation | None — blocks pipeline, escalates |
...
> **Discovery Re-entry** (triggered by Hallucination Budget breach): slice returns to Phase −1 regardless of current phase. PO + TL must revalidate intent before the pipeline resumes.
...
| Term | Definition |
|---|---|
| ASDD | Agentic Specification-Driven Development — the framework from which this manifesto is derived |
| Phase Gate | A validation checkpoint between ASDD lifecycle phases requiring human sign-off |
| Confidence Score | A 0.0–1.0 score emitted by an agent alongside every artifact |
| Hallucination Budget | The maximum tolerable rate of low-confidence agent outputs per execution wave before Discovery Re-entry is triggered |
| Discovery Re-entry | Automatic pipeline halt and slice reset to Phase −1, triggered by Hallucination Budget breach |
| Dissent Notice | A formal rejection of an agent artifact by any team member |
| Context Rot | Degradation of agent performance in large, accumulating context windows |
| Dynamic Context Injection | The Knowledge Agent's mechanism for injecting only phase-relevant context into sub-agent prompts |
| Reasoning Trace | An agent-emitted document explaining the assumptions, priorities, alternatives, and uncertainty factors behind an artifact |
| Execution Wave | A group of parallelisable implementation tasks respecting architectural layer dependencies |
| Knowledge Agent | The system memory agent responsible for state, context injection, conflict detection, learning, and drift detection |
| Ubiquitous Language | The shared domain vocabulary defined by humans and enforced across the system |
| Steering Rule | An architectural or security constraint enforced in CI and respected by all agents |
| Agent-Agent Conflict | A contradiction or overlap between outputs of two or more agents operating in parallel, requiring Knowledge Agent detection and human arbitration |

---

*Inspired by the Agile Manifesto (2001) · Proposed for the agentic software era (2026)*  
*Framework: Agentic Specification-Driven Development (ASDD) · Author: Edwin Encinas*  
*AI peer review: Claude (Anthropic) · v3.0*